...
class RaceOfficial(Observer):
                        ...
                        if self.racers[-1].getTime() - self.racers[i].getTime() <= 3000:
                            if (self.racers[-1], self.racers[i]) in self.possibleCheaters or \
                               (self.racers[-2], self.racers[i]) in self.possibleCheaters:
                                ...
                                print self.racers[-1].getBibAndName(), "and", self.racers[i].getBibAndName()
    ...
    def getCheaters(self):
        # if there are any cheaters in the list
        # send an email 
        print "Cheaters:"
        print [(i.getBibAndName(), j.getBibAndName()) for i,j in self.cheaters]
